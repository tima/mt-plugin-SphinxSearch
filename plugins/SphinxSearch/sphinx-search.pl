...
$plugin  = MT::Plugin::SphinxSearch->new(
    {
        ...
        name => 'SphinxSearch',
        description =>
          'A search script using the sphinx search engine for MySQL',
        version => $VERSION,
        ...
        settings               => MT::PluginSettings->new(
            [
                ...
                [
                    'searchd_host',
                    { Default => 'localhost', Scope => 'system' }
                ],
                [ 'searchd_port', { Default => 3312, Scope => 'system' } ],
                [
                    'searchd_pid_path',
                    { Default => '/var/log/searchd.pid', Scope => 'system' }
                ],
...
sub init_registry {
    ...
    my $reg    = {
        ...
        applications => {
            cms => { methods => { 'gen_sphinx_conf' => 'SphinxSearch::Config::gen_sphinx_conf', } },
            new_search => {
                callbacks => {
                    'sphinx_search.tag' =>
        ...
        #    'sphinx_indexer' => {
        #        name      => 'Sphinx Indexer',
        #        frequency => 2 * 60 * 60,             # every 24 hours
